(() => {
  ...
  const dataSources = [
    ...
    { id: 'wechat', name: 'WeChat', category: 'communication', type: 'Communication data', method: 'AstrBot', status: 'Ready to connect', statusType: 'idle', lastSync: 'Not connected', assets: '0 conversations', scopes: ['Chosen chats only'], purposes: ['Relationship context'], usedBy: 'Connect from Import Data catalog' },
  ...
  function setUseMissionState(nextState, options = {}) {
    ...
    if (options.announce !== false) {
      ...
      if (options.toast) showToast(useMissionSteps[useMissionStates.indexOf(nextState)]?.label || nextState);
    }
  }
  ...
  function refreshUseSidePanels() {
    ...
    document.getElementById('useOpenImportData')?.addEventListener('click', () => openPrimaryView('data'));
  ...
  function startMissionListeningDashboard() {
    ...
    if (!Recognition) { showToast('Voice capture is simulated in this browser'); return; }
    ...
    missionRecognition.onerror = event => {
      if (!['no-speech', 'aborted'].includes(event.error)) showToast('Microphone access is unavailable');
    };
    try { missionRecognition.start(); } catch (error) { showToast('Microphone access is unavailable'); }
  }
  ...
  function bindUseComposerEvents() {
    ...
    const submitPrompt = event => {
      ...
      if (!prompt) { showToast('Add a request before sending'); input?.focus(); return; }
  ...
  function bindUseMissionEvents() {
    ...
    byId('missionPromptForm')?.addEventListener('submit', event => {
      ...
      if (!prompt) { showToast('Type a request or use the microphone'); byId('missionPromptInput')?.focus(); return; }
      state.useMissionRequest = prompt;
    ...
    byId('missionConfirmRequest')?.addEventListener('click', () => { if (!state.useMissionRequest.trim()) { showToast('Add a request before continuing'); requestText?.focus(); return; } setUseMissionState('collecting'); });
    ...
    byId('missionNotify')?.addEventListener('click', event => { event.currentTarget.textContent = 'Notifications on ✓'; event.currentTarget.disabled = true; showToast('You will be notified about new investor replies'); });
    ...
    byId('missionDownload')?.addEventListener('click', () => {
      ...
      const link = document.createElement('a'); link.href = URL.createObjectURL(new Blob([report], { type: 'text/plain' })); link.download = 'weeple-investor-mission-report.txt'; link.click(); window.setTimeout(() => URL.revokeObjectURL(link.href), 1000); showToast('Mission report downloaded');
    });
    byId('missionShare')?.addEventListener('click', async () => { const text = 'Weeple found 12 investors, mailed all 12, and received 1 interested reply.'; try { if (navigator.share) await navigator.share({ title: 'Weeple mission report', text }); else { await navigator.clipboard.writeText(text); showToast('Report summary copied to clipboard'); } } catch (error) { /* User cancelled sharing. */ } });
  ...
  function openUseWorkspace(announce = true) {
    state.useWorkspaceActive = true; stopTopologyLoop(); useWorkspace.classList.add('visible'); useWorkspace.setAttribute('aria-hidden', 'false'); osShell.classList.add('use-page'); focusCluster('memory', false); renderUseMission(); if (announce) showToast('Use Data mission workspace opened');
  }
